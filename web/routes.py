...
def index():
    ...
    if request.method == "POST":  # and form.validate_on_submit():
        ...
        return __report(form, **options)
    return render_template(
        "upload_multi.html", title="Отчеты", form=form, files=request.files
    )
...
def __report(form: UploadForm, **options):
    date_purpose: DateField = form.date_purpose
    files = __upload_file()
    if files:
